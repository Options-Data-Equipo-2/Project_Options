# Proyecto Stream de datos de Massive: Datos del Mercado de Opciones de EE.UU.

## Enlaces a la API y documentación del stream

El stream de datos seleccionado proviene de la plataforma de datos **Massive** (anteriormente Polygon.io), la cual se encarga de recopilar datos financieros y proporciona acceso al mercado en tiempo real a través de APIs y WebSockets, actualizándose instantáneamente, pues obtiene datos de las bolsas financieras más importantes del mundo.
...
El stream de datos contiene información financiera en tiempo real del **mercado de opciones de Estados Unidos**. Los datos fluyen desde las 17 bolsas de opciones de EE.UU. hacia la **Options Price Reporting Authority (OPRA)**, que los consolida en un feed único y autoritativo. Massive se conecta directamente a OPRA y redistribuye los datos vía WebSocket.
...
Los datos del stream son recolectados y distribuidos por **Massive** (anteriormente Polygon.io), una empresa especializada en infraestructura de datos financieros y APIs de mercado. Massive obtiene la información directamente de OPRA y de conexiones físicas a las bolsas (Massive, 2024).

Las centrales de datos de Massive se encuentran en Nueva Jersey y cuentan con conexión directa a las siguientes bolsas de opciones:

- NYSE American Options
- Boston Options Exchange (BOX)
- Chicago Board Options Exchange (CBOE)
- Cboe EDGX Options, Cboe C2 Options, Cboe BZX Options
- Nasdaq Options Market (NOM), Nasdaq PHLX, Nasdaq BX Options
- Nasdaq ISE, Nasdaq GEMX, Nasdaq MRX
- MIAX Emerald, MIAX Pearl Options, MIAX Sapphire
- MEMX Options

El horario de mercado es de lunes a viernes, 9:30 AM a 4:00 PM Eastern Time (ET). Todos los timestamps están en UTC.
...
El procesamiento del stream de datos de mercado proporcionado por Massive no implica el manejo de datos personales, ya que la información corresponde únicamente a actividad pública del mercado financiero (precios, volumen y tiempos de transacción). Sin embargo, existen consideraciones éticas relevantes:

- **Sesgo de liquidez:** Los contratos más líquidos (SPY, AAPL, etc.) generan desproporcionadamente más eventos que contratos de baja liquidez, lo que podría sesgar análisis agregados.
- **Sesgo de supervivencia:** Solo se reciben datos de contratos actualmente listados. Contratos expirados no aparecen en el stream.
- **Sesgo temporal:** El stream solo produce datos durante horario de mercado (lunes a viernes, 9:30 AM – 4:00 PM ET).
- **Uso responsable:** Los datos se utilizan exclusivamente con fines académicos. No se realizarán operaciones financieras reales.
- **Cumplimiento regulatorio:** Se respetan los términos de uso de la API de Massive y las restricciones de redistribución de datos de OPRA.
...
Massive. (2024). Options WebSocket Overview. <https://massive.com/docs/websocket/options/overview>

Massive. (2024). Options REST API Overview. <https://massive.com/docs/rest/options/overview>
